...
import { Check, X, Star, ArrowRight, Zap, Clock, Rocket, Users, Briefcase, BookOpen, Code, ClipboardCheck, Layers, MessageSquare, ShoppingCart, LayoutDashboard, PlayCircle, Bot, Package, Globe, Monitor, Server } from "lucide-react";
...
const aiTools = [
  {name:"ChatGPT",color:"#10A37F"},{name:"Open Claw",color:"#FF6B35"},{name:"Claude Code",color:"#D4A574"},
  {name:"Bolt",color:"#8B5CF6"},{name:"Claude",color:"#D4A574"},{name:"Gemini",color:"#4285F4"},
  {name:"Loveable",color:"#FF4081"},{name:"Make",color:"#6D5CE7"},{name:"n8n",color:"#EA4B71"},
  {name:"OpenAI",color:"#10A37F"},{name:"Zapier",color:"#FF4A00"},{name:"Vercel",color:"#e5e7eb"},
  {name:"GitHub",color:"#e5e7eb"},{name:"Copilot",color:"#6CC644"},{name:"DALL·E",color:"#10A37F"},
];
...
function XItem({text}:{text:string}) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0"}}>
      <span style={{width:20,height:20,borderRadius:"50%",background:C.redDim,border:"1px solid rgba(255,68,68,0.3)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
...
function StarItem({text}:{text:string}) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0"}}>
      <Star size={14} color={C.gold} fill={C.gold} style={{flexShrink:0}}/>
      <span style={{color:"#d1d5db",fontSize:14,lineHeight:1.4}}>{text}</span>
    </div>
  );
}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
      {names.map(n => (
        <div key={n} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
          <div style={{width:64,height:64,borderRadius:14,background:"#141414",border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            {ICONS[n]?.(34) ?? <span style={{color:C.neon,fontSize:11,fontWeight:700}}>{n.slice(0,2)}</span>}
          </div>
          <span style={{fontSize:10,color:C.textMuted,textAlign:"center",lineHeight:1.3}}>{n}</span>
        </div>
      ))}
    </div>
  );
    ...
    <main style={{width:"100%",minHeight:"100vh",background:C.bg}}>
      ...
      <section style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 24px",position:"relative",overflow:"hidden",textAlign:"center"}}>
        ...
        <div style={{position:"relative",zIndex:1,maxWidth:860,width:"100%"}}>
          ...
          <p className="fade-up-2" style={{color:C.textSub,fontSize:18,maxWidth:540,margin:"0 auto 16px",lineHeight:1.6}}>
            Become a production-ready software engineer by building real-world applications.
          </p>

          <p className="fade-up-2" style={{fontWeight:800,fontSize:17,marginBottom:36,letterSpacing:"0.03em"}}>
      ...
      <section style={{padding:"96px 24px",background:"#080808"}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:16}}>
            ...
            {/* Feature cards */}
            {[
              {icon:Clock,  title:"Daily 3–4 Hours Practice", desc:"Structured hands-on coding every single day"},
              {icon:Rocket, title:"Build Real Products",       desc:"Ship production-grade apps, not toy projects"},
              {icon:Users,  title:"1:1 Mentorship",           desc:"Personal guidance from industry engineers"},
              ...
              <div key={f.title} style={cardStyle}>
                ...
                <p style={{color:C.textMuted,fontSize:13,lineHeight:1.6}}>{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(440px,1fr))",gap:20,marginBottom:20,position:"relative"}}>
            ...
            <div style={cardStyle}>
              ...
              <p style={{color:C.textMuted,fontSize:13,marginBottom:24}}>Master the React ecosystem & ship to production.</p>
              <TechGrid names={frontendTechs}/>
            </div>

            <div style={cardStyle}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
                <Zap size={18} color={C.neon}/>
                <h3 style={{color:C.neon,fontWeight:800,fontSize:14,letterSpacing:"0.1em",textTransform:"uppercase"}}>Learning Outcomes</h3>
              </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(440px,1fr))",gap:20,position:"relative"}}>
            ...
            <div style={cardStyle}>
              ...
              <p style={{color:C.textMuted,fontSize:13,marginBottom:24}}>Scalable APIs, databases & cloud deployment.</p>
      ...
      <section style={{padding:"96px 24px",background:C.bg}}>
        <div style={{maxWidth:960,margin:"0 auto"}}>
          <div style={{textAlign:"center",marginBottom:52}}>
            ...
            <h2 style={{fontSize:"clamp(26px,3.5vw,44px)",fontWeight:900,color:"#fff",marginBottom:10,marginTop:4}}>Powered by AI Engineering Tools</h2>
            <p style={{color:C.textSub,fontSize:15,maxWidth:500,margin:"0 auto"}}>
              Don&apos;t just write code. Generate, debug, and scale with the best AI tools.
            </p>
          </div>
